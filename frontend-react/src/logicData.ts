...
export const COPY: Record<string, GameCopy> = {
  ...
  "argument": {
    ...
    "levels": {
      "kids": "Building an argument is like stacking blocks: your reasons are the blocks at the bottom, and your conclusion sits on top. If even one bottom block is wobbly — not really true — the whole tower can fall, even if everything above it was stacked perfectly! Try clicking the blocks in the tower above to knock one out and watch the claim topple, then pick a different example to explore.",
      ...
      "adult": "This is the actual skeleton behind persuasive legal briefs, grant proposals, and policy memos, whether or not the author has heard of Toulmin. A memo that states a claim and cites evidence but never states the warrant — why that evidence is supposed to support that claim — asks a skeptical reader to take the connection on faith, which is exactly where careful readers push back. Load one of the historical examples above, or draft your own real argument and see it laid out the same way."
    }
  },
  "circuit": {
    "title": "Circuit Builder — Wire Your Own Logic Chip",
    "subtitle": "Place AND, OR, NOT, XOR, NAND, and NOR gates and wire them together to build a working circuit — the same building block every computer chip on Earth is made from, just with billions more of them.",
    "prereq": "**Math you’ll use:** *Boolean algebra & circuit design* — the same math as the Logic Gates game above, but here you place real gates and wire them together instead of typing a formula. Toggle an input and watch the signal (0 or 1) light its way through every wire to the output bulb.",
    "anecdote": "**Historical note:** Claude Shannon's 1937 master's thesis showed that Boolean algebra — logic — could be built directly out of electrical switches, launching the digital age. A striking fact this game lets you prove for yourself: **NAND alone is \"universal\"** — every other gate, including NOT, AND, OR, and XOR, can be built using nothing but NAND gates wired together. That's not a curiosity — most real chip designs are built almost entirely from one or two universal gate types, because it's cheaper to manufacture millions of identical gates than several different kinds.\n\n**Social usage:** Every logic gate in a real CPU works exactly like the ones here, just smaller — a modern chip packs billions of them onto a sliver of silicon the size of a fingernail.",
    "prompt": "Explain logic gates, Boolean circuits, and why NAND is a universal gate that can build any other logic gate",
    "levels": {
      "kids": "A logic gate is a tiny decision-maker: it looks at 0s and 1s coming in and spits out a 0 or a 1 depending on its rule. An AND gate is a strict teacher — everyone has to say \"yes\" (1) for it to say yes. An OR gate is easygoing — just ONE yes is enough. A NOT gate is a rebel — it always says the opposite of what you tell it! Wire a few together and you've built your own tiny computer brain.",
      "teen": "Every gate is just a tiny function from 0s/1s to a 0/1: AND needs both inputs true, OR needs at least one, XOR needs exactly one (not both), and NOT flips whatever it's given. NAND and NOR are AND/OR with the answer flipped at the end (that's the little bubble on their output). Chain enough of these together — the same way you'd chain arithmetic operations — and you can compute anything a circuit needs to: addition, comparisons, even a full CPU.",
      "college": "This game lets you build the standard basis proofs by hand instead of just reading them: {AND, OR, NOT} is functionally complete (any Boolean function is expressible using only these three — try the XOR challenge), and {NAND} alone is *also* functionally complete (try building NOT from a single NAND first, then notice AND and OR are just as reachable from there). Circuit minimization — finding the fewest gates for a given truth table — is exactly the optimization problem real chip synthesis tools (Yosys, ABC) solve at industrial scale using SAT solvers and And-Inverter Graphs.",
      "adult": "This is a miniature version of what hardware description languages (Verilog, VHDL) compile down to, and what a chip designer's schematic capture tool looks like at the gate level. The \"NAND is universal\" property you can prove here directly is why standard-cell chip libraries are built almost entirely from a small handful of gate types — uniformity is cheaper to fabricate and verify at scale than variety, even though it sometimes costs a few extra gates per circuit."
    }
  }
};
