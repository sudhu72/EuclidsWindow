/* AI by Hand — Feynman discovery walkthroughs.
   Inspired by Tom Yeh's "AI by Hand" (work tiny examples by hand) and Feynman's
   method (motivate → work a concrete case → derive the rule yourself → explain
   it simply → connect it). Content is curated & static; every number here was
   verified in NumPy so the by-hand arithmetic is correct.

   Each exercise has six stages:
     know     — what you already know (the basic math it rests on)
     question — the problem that makes you want to invent this
     byhand   — a tiny worked numerical example, step by step
     discover — generalize the example into the rule/formula (you derive it)
     explain  — the Feynman one-breath explanation
     connect  — links down to prerequisites and up to what it unlocks
*/
(function () {
  "use strict";

  const M = (s) => `\\(${s}\\)`; // inline math shorthand for readability below
  ...
  const TIERS = ["Foundations", "Sequences & memory", "Attention & Transformers", "Generative & advanced"];
  const STAGE_ORDER = [
  ...
  function renderGallery(root) {
    const byTier = TIERS.map((tier) => ({
      tier,
      items: EXERCISES.filter((e) => e.tier === tier),
    }));
    root.innerHTML = `
      <div class="abh-intro">
        <h3>AI by Hand — discover it yourself</h3>
        <p>19 ideas, each rebuilt the Feynman way: start from math you know, work a
        <b>tiny example by hand</b>, then <b>derive the rule yourself</b> and see how it
        connects — basic to advanced. Every number here is verified. Pick one and build it.</p>
      </div>
      ${byTier.map((g) => `
        <div class="abh-tier">
          <h4 class="abh-tier-title">${g.tier}</h4>
          <div class="abh-grid">
            ${g.items.map((e) => `
              <button class="abh-card" data-id="${e.id}">
                <span class="abh-card-title">${e.title}</span>
                <span class="abh-card-one">${e.oneLiner}</span>
                <span class="abh-card-pre">needs: ${e.prereqs.join(" · ")}</span>
              </button>`).join("")}
          </div>
        </div>`).join("")}
    `;
    root.querySelectorAll(".abh-card").forEach((btn) =>
      btn.addEventListener("click", () => renderExercise(root, btn.dataset.id))
    );
  }
  ...
  function renderExercise(root, id) {
    ...
    if (!e) return;
    const s = e.stages;
    const stageHtml = STAGE_ORDER.map(([key, label]) => `
      <div class="abh-stage">
        <div class="abh-stage-h">${label}</div>
        <div class="abh-stage-b">${s[key]}</div>
      </div>`).join("");
    const exploreQ = `How do I derive ${e.title} from first principles with a tiny worked example?`;
    root.innerHTML = `
      <button class="abh-back">← All exercises</button>
      <div class="abh-ex-head">
        <span class="abh-tier-badge">${e.tier}</span>
        <h3>${e.title}</h3>
        <p class="abh-ex-one">${e.oneLiner}</p>
        <p class="abh-ex-pre">Builds on: ${e.prereqs.join(" · ")}</p>
      </div>
      ${stageHtml}
      <div class="abh-stage abh-connect">
        <div class="abh-stage-h">⑥ Connections (basic → advanced)</div>
        <div class="abh-stage-b">
          <b>Rests on:</b> ${s.connect.back.join(" · ")}<br>
          <b>Unlocks:</b> ${s.connect.forward.join(" · ")}
        </div>
      </div>
      <div class="abh-explore">
        <a href="#" class="abh-explore-link" data-q="${encodeURIComponent(exploreQ)}">Discover this with the tutor →</a>
      </div>
    `;
    root.querySelector(".abh-back").addEventListener("click", () => renderGallery(root));
    const link = root.querySelector(".abh-explore-link");
    if (link) link.addEventListener("click", (ev) => {
      ev.preventDefault();
      const q = decodeURIComponent(link.dataset.q);
      if (window.switchToTab) window.switchToTab("tutor");
      if (window.sendTutorQuestion) window.sendTutorQuestion(q);
    });
    typeset(root);
    root.scrollTop = 0;
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const root = document.getElementById("aibyhand-root");
